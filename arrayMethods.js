...
const postTaxPrices = prices.map((price) => {
  let priceWithTax = price * (1 + 0.07);
  let parsedPrice = priceWithTax.toFixed(2);
...
// CODE HERE
const ordersAfterTax = orders.map((elem) => {
  let updatedPrice = elem["price"] * (1 + elem["tax"]);
  let priceDecimalReduced = updatedPrice.toFixed(2);
  return Number(priceDecimalReduced);
});
