...
const uniq = (arr, callback) => {
  let dupsRemovedArray = [];

  for (let i = 0; i < arr.length; i++) {
    for (let j = 0; j < arr.length; j++) {
      if (i !== j) {
        if (!dupsRemovedArray.includes(arr[i])) {
          dupsRemovedArray.push(arr[i]);
        }
      }
    }
  }
  return callback(dupsRemovedArray);
};
...
// CODE HERE
const addingFactory = (num) => (anotherNum) => num + anotherNum;
/*
